...
|                     | Count   |
| ------------------- | ------- |
| Iteration-one tasks | 89      |
| Deferred tasks      | 17      |
| **Total**           | **106** |

Task `028` is `in-progress`. 20 of 89 iteration-one tasks are `complete`.
...
| `006` | Threat model the operator tooling that writes outside the request path | Foundation                           | `027`                                     | `pending`     | —         | —       |
...
| `028` | Purge reaches storage objects, and assets join the lifecycle           | Data, authorization, audit           | `026`                                     | `in-progress` | —         | —       |
